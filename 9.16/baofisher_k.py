...
# SciPy >= 1.14 removed `cumtrapz` in favour of `cumulative_trapezoid`.
if not hasattr(scipy.integrate, "cumtrapz") and hasattr(
    scipy.integrate, "cumulative_trapezoid"
):
    scipy.integrate.cumtrapz = scipy.integrate.cumulative_trapezoid

# SciPy >= 1.14 renamed `simps` to `simpson` and, depending on version,
# may make `x` keyword-only. Provide a wrapper with the old signature.
...
EXP_OVERFLOW_VAL = 250.0  # Max. value of exponent for np.exp() before assuming overflow
...
CAMB_KMAX = 20.0 / 0.7  # Max. k for CAMB, in h Mpc^-1


def _resolve_camb_exec_dir() -> str | None:
    """Resolve the directory containing the CAMB executable ('./camb').

    Resolution order:
    1) Environment variables: CAMB_EXEC_DIR or BAO21CM_CAMB_EXEC
       (can be either the directory containing 'camb' or the full path to 'camb').
    2) Auto-detect a CAMB checkout in parent directories of this file.
       (Looks for CAMB/fortran or CAMB.)
    """

    env = os.environ.get("CAMB_EXEC_DIR") or os.environ.get("BAO21CM_CAMB_EXEC")
    if env:
    ...
    for parent in [here.parent, *here.parents]:
        for cand in (parent / "CAMB" / "fortran", parent / "CAMB"):
            if cand.is_dir():
                return str(cand)
    return None


# Directory containing camb executable. Prefer env var / auto-detect.
CAMB_EXEC = _resolve_camb_exec_dir() or "/home/phil/oslo/bao21cm/camb"
...
def ellipse_for_fisher_params(p1, p2, F, Finv=None):
    ...
    y1 = 0.5 * (c11 + c22)
    y2 = np.sqrt(0.25 * (c11 - c22) ** 2.0 + c12**2.0)
    a = 2.0 * np.sqrt(y1 + y2)  # Factor of 2 because def. is *total* width of ellipse
    b = 2.0 * np.sqrt(y1 - y2)
    ...
    if c11 != c22:
        ang = 0.5 * np.arctan(2.0 * c12 / (c11 - c22))
    else:
        ang = 0.5 * np.arctan(2.0 * c12 / 1e-20)  # Sign sensitivity here

    # Factors to use if 1,2,3-sigma contours required
    alpha = [1.52, 2.48, 3.44]

    return w, h, ang * 180.0 / np.pi, alpha
...
def plot_ellipse(F, p1, p2, fiducial, names, ax=None):
    ...
    ellipses = [
        matplotlib.patches.Ellipse(
            xy=(x, y), width=alpha[k] * b, height=alpha[k] * a, angle=ang, fc="none"
        )
        for k in range(0, 2)
    ]
    if ax is None:
    ...
    for i in range(N):
        for j in range(N):
            ...
            if i == j:
                ...
                _x = np.linspace(u - 4.0 * std, u + 4.0 * std, 500)
                _y = np.exp(-0.5 * (_x - u) ** 2.0 / std**2.0) / np.sqrt(
                    2.0 * np.pi * std**2.0
                )
                P.plot(_x, _y, "r-")
                ...
                if priors is not None:
                    if not np.isinf(priors[i]):
                        std = priors[i]
                        yp = np.exp(
                            -0.5 * (_x - u) ** 2.0 / priors[i] ** 2.0
                        ) / np.sqrt(2.0 * np.pi * std**2.0)
...
def fix_log_plot(y, yerr):
    ...
    yup = yerr.copy()
    ylow = yerr.copy()
    ylow[np.where(y - yerr <= 0.0)] = y[np.where(y - yerr <= 0.0)] * 0.99999
...
def plot_corrmat(F, names):
    ...
    # F_corr = F_corr**3.
    matshow = ax.matshow(
        F_corr, vmin=-1.0, vmax=1.0, cmap=matplotlib.cm.get_cmap("RdBu")
    )
...
def zbins_fixed(expt, zbin_min=0.0, zbin_max=6.0, dz=0.1):
    """
    ...
    zmin = expt["nu_line"] / expt["survey_numax"] - 1.0
    zmax = expt["nu_line"] / (expt["survey_numax"] - expt["survey_dnutot"]) - 1.0
...
def zbins_const_dr(expt, cosmo, bins=None, nsamples=500, initial_dz=None):
    ...
    _z = np.linspace(0.0, zmax, nsamples)
    a = 1.0 / (1.0 + _z)
    H0 = 100.0 * cosmo["h"]
    w0 = cosmo["w0"]
    wa = cosmo["wa"]
    ...
    omegaDE = ol * np.exp(3.0 * wa * (a - 1.0)) / a ** (3.0 * (1.0 + w0 + wa))
    E = np.sqrt(om * a ** (-3.0) + ok * a ** (-2.0) + omegaDE)
    _r = (C / H0) * np.concatenate(([0.0], scipy.integrate.cumtrapz(1.0 / E, _z)))
...
def zbins_const_dnu(expt, cosmo, bins=None, dnu=None, initial_dz=None):
    ...
    zmin = expt["nu_line"] / expt["survey_numax"] - 1.0
    zmax = expt["nu_line"] / (expt["survey_numax"] - expt["survey_dnutot"]) - 1.0
    numax = expt["nu_line"] / (1.0 + zmin)
    numin = expt["nu_line"] / (1.0 + zmax)

    # nu as a function of z
    nu_z = lambda zz: expt["nu_line"] / (1.0 + zz)
    z_nu = lambda f: expt["nu_line"] / f - 1.0
    ...
    if bins is not None:
        ...
        dnu = -1.0 * np.abs(dnu)  # dnu negative, to go from highest to lowest freq.
...
def zbins_split_width(expt, dz=(0.1, 0.3), zsplit=2.0):
    """
...
def load_interferom_file(fname):
    ...
    interp_nx = scipy.interpolate.interp1d(
        x, _nx, kind="linear", bounds_error=False, fill_value=1.0 / INF_NOISE
    )
    return interp_nx
...
def overlapping_expts(expt_in, zlow=None, zhigh=None, Sarea=None):
    ...
    if "overlap" in list(expt_in.keys()):
        ...
        if Sarea is not None:
            expt_in["Sarea"] = Sarea  # Override Sarea
        return expt_in
    ...
    if zlow is None or zhigh is None:
        ...
        if Sarea is not None:
            expt["Sarea"] = Sarea  # Override Sarea
        return expt

    # Calculate bin min/max freqs.
    nu_high = expt1["nu_line"] / (1.0 + zlow)
    nu_low = expt1["nu_line"] / (1.0 + zhigh)
    ...
    if nu_low < 0.9999 * nu1[0] or nu_high > 1.0001 * nu1[1]:
        N1 = 0
    if nu_low < 0.9999 * nu2[0] or nu_high > 1.0001 * nu2[1]:
        N2 = 0
    assert N1 + N2 != 0
...
def load_power_spectrum(
    cosmo, cachefile, kmax=CAMB_KMAX, comm=None, force=False, force_load=False
):
    """
    ...
    p["transfer_kmax"] = kmax / cosmo["h"] if kmax <= 14.0 else 14.0
    ...
    if myid == 0:
        ...
        dat = cached_camb_output(
            p, cachefile, mode="matterpower", force=force, force_load=force_load
        )
    if comm is not None:
        comm.barrier()
    if myid != 0:
...
def fgrowth(cosmo, z, usegamma=False):
    ...
    """
    c = cosmo
    Oma = c["omega_M_0"] * (1.0 + z) ** 3.0 / Ez(cosmo, z) ** 2.0
    a = 1.0 / (1.0 + z)
    ...
    if "gamma0" not in list(c.keys()) or usegamma == True:
        ...
        gamma = c["gamma0"] + c["gamma1"] * (1.0 - a)
    eta = 0.0 if "eta0" not in list(c.keys()) else (c["eta0"] + c["eta1"] * (1.0 - a))
    f = Oma**gamma * (1.0 + eta)
    return f
...
def Ez(cosmo, z):
    ...
    ok = 1.0 - om - ol
    omegaDE = ol * np.exp(3.0 * wa * (a - 1.0)) / a ** (3.0 * (1.0 + w0 + wa))
    return np.sqrt(om * a ** (-3.0) + ok * a ** (-2.0) + omegaDE)


def background_evolution_splines(cosmo, zmax=10.0, nsamples=500):
    """
    ...
    r_c = np.concatenate(([0.0], scipy.integrate.cumtrapz(1.0 / E, _z)))
    if ok > 0.0:
    ...
    _D = np.concatenate(
        (
            ...
            scipy.integrate.cumtrapz(_f, np.log(a)),
        )
    )
    _D = np.exp(_D)
...
def fsigma8_for_params(z, cosmo, zmax=10.0, usegamma=False):
    """
    Calculate f(z)*sigma_8(z) for a set of cosmological parameters.
    """
    _z = np.linspace(0.0, zmax, 400)
    a = 1.0 / (1.0 + _z)
    ...
    # Construct interpolating functions and return
    fs8 = scipy.interpolate.interp1d(
        _z, _f * _D * cosmo["sigma_8"], kind="linear", bounds_error=False
    )
    return fs8(z)
...
def fsigma8_derivs(z, cosmo, params=[], dx=[]):
    ...
    for i in range(len(params)):
        ...
        derivs.append((fs8p - fs8m) / (2.0 * dx[i]))
    return derivs
...
def Tb(z, cosmo, formula="powerlaw"):
    ...
    if formula == "santos":
        Tb = 0.1376 * (1.0 + 1.44 * z - 0.277 * z**2.0)
    elif formula == "powerlaw":
        Tb = 5.5919e-02 + 2.3242e-01 * z - 2.4136e-02 * z**2.0
    elif formula == "chang":
        Tb = 0.3 * (omegaHI / 1e-3) * np.sqrt(0.29 * (1.0 + z) ** 3.0)
        Tb *= np.sqrt((1.0 + z) / 2.5)
        Tb /= np.sqrt(cosmo["omega_M_0"] * (1.0 + z) ** 3.0 + cosmo["omega_lambda_0"])
    else:
        # From Hall et al.
        om = cosmo["omega_M_0"]
        ol = cosmo["omega_lambda_0"]
        ok = 1.0 - om - ol
        E = np.sqrt(om * (1.0 + z) ** 3.0 + ok * (1.0 + z) ** 2.0 + ol)
        Tb = 188.0 * cosmo["h"] * omegaHI * (1.0 + z) ** 2.0 / E
    Tb *= 1.0 if "Tb_factor" not in list(cosmo.keys()) else cosmo["Tb_factor"]
    return Tb
...
def bias_HI(z, cosmo, formula="powerlaw"):
    ...
    """
    if formula == "old":
        return cosmo["bHI0"] * (1.0 + 3.80e-1 * z + 6.7e-2 * z**2.0)
    else:
        return (cosmo["bHI0"] / 0.677105) * (
            6.6655e-01 + 1.7765e-01 * z + 5.0223e-02 * z**2.0
        )
...
def omega_HI(z, cosmo, formula="powerlaw"):
    ...
    if formula == "old":
        ...
        return cosmo["omega_HI_0"] * (1.0 + 4.77e-2 * z - 3.72e-2 * z**2.0)
    else:
        return (cosmo["omega_HI_0"] / 0.000486) * (
            4.8304e-04 + 3.8856e-04 * z - 6.5119e-05 * z**2.0
        )
...
def convert_to_camb(cosmo):
    ...
    """
    p = {}
    p["hubble"] = 100.0 * cosmo["h"]
    p["omch2"] = (cosmo["omega_M_0"] - cosmo["omega_b_0"]) * cosmo["h"] ** 2.0
    p["ombh2"] = cosmo["omega_b_0"] * cosmo["h"] ** 2.0
    p["omk"] = 1.0 - (cosmo["omega_M_0"] + cosmo["omega_lambda_0"])
...
def cached_camb_output(
    p, cachefile, cosmo=None, mode="matterpower", force=False, force_load=False
):
    """
    ...
    for key in keys:
        if key != "output_root":
            m.update(f"{key}:{p[key]}".encode("utf-8"))
    in_hash = m.hexdigest()
    ...
    try:
        ...
        if in_hash != hash and not force and not force_load:
            print(
                "\tcached_camb_output: Hashes do not match; delete the cache file and run again to update."
            )
    ...
    if mode == "transfer":
        ...
        dat[0] *= h  # Convert h^-1 Mpc => Mpc
        dat[1] *= (sigma_8_in / sigma8) ** 2.0 / h**3.0
...
def deriv_transfer(cosmo, cachefile, kmax=CAMB_KMAX, kref=1e-3, comm=None, force=False):
    ...
    scale_fn = 1.0 / (Tk * k**2.0)
    iscalefn = scipy.interpolate.interp1d(
        k, scale_fn, kind="linear", bounds_error=False, fill_value=0.0
    )
    ...
    idscalefn_dk = scipy.interpolate.interp1d(
        k, dscalefn, kind="linear", bounds_error=False, fill_value=0.0
    )
...
def deriv_neutrinos(
    ...
    if comm is not None:
        ...
        size = comm.Get_size()
    if myid == 0:
        print("\tderiv_neutrinos(): Loading P(k) data for derivs...")

    # Set finite difference values and other CAMB parameters
    p = convert_to_camb(cosmo)
    p["transfer_kmax"] = kmax if kmax <= 10.0 else 10.0
    if mnu != 0.0:
    ...
    dPk_dp = (dat[2][1][:idxmin] - dat[0][1][:idxmin]) / (2.0 * dp)
    dlogPk_dp = dPk_dp / dat[1][1][:idxmin]
    k = dat[0][0][:idxmin]

    # Interpolate result
    idlogpk = scipy.interpolate.interp1d(
        k, dlogPk_dp, kind="linear", bounds_error=False, fill_value=dlogPk_dp[-1]
    )
    return idlogpk
...
def logpk_derivative(pk, kgrid):
    ...
    dP[np.where(np.isnan(dP))] = 1.0  # Set NaN values to 1 (sets deriv. to zero)
...
def fbao_derivative(fbao, kgrid, kref=[3e-2, 4.5e-1]):
    ...
    if len(idxs[0]) > 0:
        dfbao_dk[idxs] = 0.0
        dfbao_dk[np.max(idxs) + 1] = 0.0

    # Interpolate
    idfbao_dk = scipy.interpolate.interp1d(
        kgrid, dfbao_dk, kind="linear", bounds_error=False, fill_value=0.0
    )
    return idfbao_dk
...
def spline_pk_nobao(k_in, pk_in, kref=[2.4e-2, 4.5e-1]):
    ...
    _interp_pk = scipy.interpolate.interp1d(
        np.log(k_in), np.log(pk_in), kind="quadratic", bounds_error=False
    )
    ...
    _pk_smooth = scipy.interpolate.UnivariateSpline(
        np.log(k_in[idxs]), np.log(pk_in[idxs]), k=3, s=0
    )
    ...
    fwiggle = scipy.interpolate.UnivariateSpline(
        k_in, pk_in / pk_smooth(k_in), k=3, s=0
    )
    ...
    ipk = scipy.interpolate.interp1d(
        k_in, pk_nobao, kind="linear", bounds_error=False, fill_value=0.0
    )
    ifk = scipy.interpolate.interp1d(
        k_in, fk, kind="linear", bounds_error=False, fill_value=0.0
    )
    return ipk, ifk
...
def integrate_grid_cumulative(integrand, kgrid, ugrid):
    ...
    if DBG_PLOT_CUMUL_INTEGRAND:
        ...
        P.xscale("log")
        P.show()
    return scipy.integrate.cumtrapz(Ik, kgrid, initial=0.0)
...
def integrate_fisher_elements(derivs, kgrid, ugrid):
    ...
    for i in range(Nparams):
        for j in range(Nparams):
            if j >= i:
                F[i, j] = integrate_grid(K**2.0 * derivs[i] * derivs[j], kgrid, ugrid)
                F[j, i] = F[i, j]
    return F
...
def integrate_fisher_elements_cumulative(pbinned, derivs, kgrid, ugrid):
    ...
    integ = [
        integrate_grid_cumulative(K**2.0 * derivs[i] * derivs[p], kgrid, ugrid)
        for i in range(Nparams)
    ]
    return integ
...
def bin_cumulative_integrals(cumul, kgrid, kedges):
    ...
    for i in range(Nparams):
        ...
        y_k = scipy.interpolate.interp1d(
            _kgrid, _cumul, kind="linear", bounds_error=False, fill_value=0.0
        )
        ybins = y_k(kedges)
...
def noise_rms_per_voxel(z, expt):
    ...
    """
    Tsky = (
        60e3 * (300.0 * (1.0 + z) / expt["nu_line"]) ** 2.55
    )  # Foreground sky signal (mK)
    Tsys = expt["Tinst"] + Tsky  # System temperature
    I = 1.0 / (expt["Ndish"] * expt["Nbeam"])  # Dish multiplicity
    theta_fwhm = 3e8 * (1.0 + z) / (1e6 * expt["nu_line"]) / expt["Ddish"]  # Beam FWHM

    sigma_T = (
        Tsys
        * np.sqrt(expt["Sarea"] * I)
        / np.sqrt(expt["dnu"] * expt["ttot"] * theta_fwhm**2.0)
    )
    return sigma_T
...
def noise_rms_per_voxel_interferom(z, expt):
    ...
    nu = expt["nu_line"] / (1.0 + z)
    l = 3e8 / (nu * 1e6)
    fov = (l / expt["Ddish"]) ** 2.0

    sigma_T = (
        Tsys * np.sqrt(expt["Sarea"] * fov**2.0) / np.sqrt(expt["dnu"] * expt["ttot"])
    )
    return sigma_T


def noise_k_limits(cosmo, expt):
    """Return the foreground |k_parallel| floor and total-k ceiling (Mpc^-1).

    ``expt['kmax']`` overrides only the total-k ceiling. If omitted, the
    original redshift-dependent nonlinear cutoff is retained. Baseline and
    field-of-view restrictions are independent of this override.
    """
    kfg = (
        2.0 * np.pi * expt["nu_line"]
        / (expt["survey_dnutot"] * cosmo["rnu"])
        * expt.get("kfg_fac", 1.0)
    )
    if "kmax" in expt:
        kmax = float(expt["kmax"])
        if not np.isfinite(kmax) or kmax <= 0.0:
            raise ValueError("expt['kmax'] must be finite and positive (Mpc^-1).")
    else:
        kmax = expt.get("k_nl0", 0.14) * (1.0 + cosmo["z"]) ** (
            2.0 / (2.0 + cosmo["ns"])
        )
    return kfg, kmax


def interferometer_mode_mask(q, y, cosmo, expt):
    """Identify measured modes without interpreting INF_NOISE as a power.

    Inputs use the same angular/frequency Fourier coordinates as ``Cnoise``.
    The broadcast boolean result includes the original baseline, FoV and
    foreground cuts, plus the total-k ceiling from ``noise_k_limits``. Redshift
    band coverage must be checked by the caller. An interpolated positive
    density remains valid even close to zero; no arbitrary noise floor is
    imposed. Integrals of inverse density require an edge-convergence check.
    """
    if not expt["mode"].startswith("i"):
        raise ValueError("interferometer_mode_mask requires interferometer mode.")
    q, y = np.broadcast_arrays(np.asarray(q, dtype=float), np.asarray(y, dtype=float))
    kperp = q / (cosmo["aperp"] * cosmo["r"])
    kpar = y / (cosmo["apar"] * cosmo["rnu"])
    nu = expt["nu_line"] / (1.0 + cosmo["z"])
    wavelength = 3e8 / (nu * 1e6)
    u = kperp * cosmo["r"] / (2.0 * np.pi)
    valid = np.isfinite(kperp) & np.isfinite(kpar) & (kperp >= 0.0)

    if "n(x)" in expt:
        baseline = expt["n(x)"]
        x = u / nu
        density = np.asarray(baseline(x))
        # The interpolation sentinel is a missing-mode marker, not coverage.
        valid &= np.isfinite(density) & (density > 1.0 / INF_NOISE)
        if hasattr(baseline, "x"):
            valid &= (x >= baseline.x[0]) & (x <= baseline.x[-1])
    else:
        valid &= (u >= expt["Dmin"] / wavelength)
        valid &= (u <= expt["Dmax"] / wavelength)

    if "cyl" not in expt["mode"]:
        valid &= u >= expt["Ddish"] / wavelength

    kfg, kmax = noise_k_limits(cosmo, expt)
    valid &= np.abs(kpar) >= kfg
    valid &= np.hypot(kperp, kpar) <= kmax
    return valid
...
def interferometer_response(q, y, cosmo, expt):
    ...
    u = kperp * c["r"] / (2.0 * np.pi)  # UV plane: |u| = d / lambda
    nu = expt["nu_line"] / (1.0 + c["z"])
    l = 3e8 / (nu * 1e6)  # Wavelength (m)
    Ddish = expt["Ddish"]

    # Calculate FOV (180deg * theta for cylinder mode)
    fov = np.pi * (l / Ddish) if "cyl" in expt["mode"] else (l / Ddish) ** 2.0
    ...
    if "n(x)" in list(expt.keys()):
        ...
        x = u / nu  # x = u / (freq [MHz])
        n_u = expt["n(x)"](x) / nu**2.0  # n(x) = n(u) * nu^2
        n_u[np.where(n_u == 0.0)] = 1.0 / INF_NOISE
    else:
        ...
        ff = expt["Ndish"] * (expt["Ddish"] / expt["Dmax"]) ** 2.0  # Filling factor
        print("\tArray filling factor: %3.3f" % ff)
        if ff > 1.0:
            raise ValueError(
                (
                    "Filling factor is > 1; dishes are too big to "
        ...
        n_u = (
            ...
            * l**2.0
            * np.ones(u.shape)
            / (2.0 * np.pi * (expt["Dmax"] ** 2.0 - expt["Dmin"] ** 2.0))
        )
        n_u[np.where(u < u_min)] = 1.0 / INF_NOISE
        n_u[np.where(u > u_max)] = 1.0 / INF_NOISE

    # FOV cut-off (disabled for cylinders)
    if "cyl" not in expt["mode"]:
        l = 3e8 / (nu * 1e6)  # Wavelength (m)
        u_fov = 1.0 / np.sqrt(fov)
        n_u[np.where(u < u_fov)] = 1.0 / INF_NOISE

    # Gaussian in parallel direction. Perp. direction already accounted for by
    # n(u) factor in multiplicity (I)
    sigma_kpar = np.sqrt(16.0 * np.log(2)) * expt["nu_line"] / (expt["dnu"] * c["rnu"])
    B_par = (y / (c["rnu"] * sigma_kpar)) ** 2.0
...
def dish_response(q, y, cosmo, expt):
    ...
    l = 3e8 * (1.0 + c["z"]) / (1e6 * expt["nu_line"])
    theta_fwhm = l / expt["Ddish"]
    sigma_kpar = np.sqrt(16.0 * np.log(2)) * expt["nu_line"] / (expt["dnu"] * c["rnu"])
    sigma_kperp = np.sqrt(16.0 * np.log(2)) / (c["r"] * theta_fwhm)

    # Sanity check: Require that Sarea > Nbeam * (beam)^2
    if expt["Sarea"] < expt["Nbeam"] / (sigma_kperp * c["r"]) ** 2.0:
    ...
    B_tot = (q / (c["r"] * sigma_kperp)) ** 2.0 + (y / (c["rnu"] * sigma_kpar)) ** 2.0
...
def Cnoise(q, y, cosmo, expt, cv=False):
    ...
    npol = expt["npol"] if "npol" in list(expt.keys()) else 2.0
    ...
    Tsky = (
        60e3 * (300.0 * (1.0 + c["z"]) / expt["nu_line"]) ** 2.55
    )  # Temp. of sky (mK)
    Tsys = expt["Tinst"] + Tsky
    if "Tsky_factor" in list(expt.keys()):
        Tsys += expt["Tsky_factor"] * Tsky
    noise = Tsys**2.0 * Vsurvey / (npol * expt["ttot"] * expt["dnutot"])
    if cv:
        noise = 1.0  # Cosmic variance-limited calc.
    ...
    if expt["mode"][0] == "i":
        ...
        Aeff = (
            effic * 0.25 * np.pi * expt["Ddish"] ** 2.0
            if "Aeff" not in list(expt.keys())
            else expt["Aeff"]
        )
        ...
        noise *= l**4.0 / (expt["Nbeam"] * (Aeff * theta_b) ** 2.0)
        ...
        if "paf" in expt["mode"]:
            ...
            print("(PAF)")
            noise *= l**4.0 / (Aeff**2.0 * theta_b**4.0)
            noise *= 1.0 if nu > expt["nu_crit"] else (expt["nu_crit"] / nu) ** 2.0
        elif "hybrid" in expt["mode"]:
            print("(hybrid array)")
            # Standard autocorrelation mode, but with overlapping sub-arrays
            # Calculate properties of sub-array 2
            Aeff2 = (
                effic2 * 0.25 * np.pi * expt["Ddish2"] ** 2.0
                if "Aeff2" not in list(expt.keys())
                else expt["Aeff2"]
            )
            ...
            if nu >= numin1 and nu <= numax1:
                if nu >= numin2 and nu <= numax2:
                    ...
                    avg_AoverT = ((Nd1 * Aeff / Tsys) + (Nd2 * Aeff2 / Tsys2)) / float(
                        Ndish_comb
                    )
                else:
                    # Only array 1
                ...
                # No overlap; error
                raise ValueError(
                    "Cnoise(): hybrid array: neither array covers this frequency"
                )
            # Noise expression
            noise *= l**4.0 / (avg_AoverT**2.0 * theta_b_comb**4.0)

            # Replace previous 1-array values with new 2-array values
            noise *= (expt["Ndish"] / float(Ndish_comb)) / Tsys**2.0
        else:
            # Standard dish autocorrelation mode
            print("(dish)")
            noise *= l**4.0 / (Aeff**2.0 * theta_b**4.0)

        noise *= 1.0 / (expt["Ndish"] * expt["Nbeam"])
    ...
    """
    # FIXME
    if "comb" in expt["mode"]:
        raise NotImplementedError("Combined mode not implemented!")

    # Preserve the foreground floor independently of the total-k ceiling.
    kfg, kmax = noise_k_limits(cosmo, expt)
    noise[np.where(np.abs(kpar) < kfg)] = INF_NOISE

    # Use an explicit analysis ceiling when requested; otherwise retain the
    # original nonlinear prescription (Smith et al. 2003, Eq. 20).
    if "kmax" not in expt and "k_nl0" not in expt:
        print("\tNonlinear scale not specified; setting k_NL,0 = 0.14 Mpc^-1")
    noise[np.where(np.sqrt(kpar**2.0 + kperp**2.0) > kmax)] = INF_NOISE
...
def Csignal(q, y, cosmo, expt):
    ...
    if RSD_FUNCTION == "kaiser":
        # Pedro's notes, Eq. 7
        Frsd = (c["btot"] + f * u2) ** 2.0 * np.exp(-u2 * (k * c["sigma_nl"]) ** 2.0)
    else:
        # arXiv:0812.0419, Eq. 5
        sigma_nl2_eff = (D * c["sigma_nl"]) ** 2.0 * (1.0 - u2 + u2 * (1.0 + f) ** 2.0)
        Frsd = (c["btot"] + f * u2) ** 2.0 * np.exp(-0.5 * k**2.0 * sigma_nl2_eff)

    # Construct signal covariance and return
    cs = Frsd * (1.0 + c["A"] * c["fbao"](k)) * D**2.0 * c["pk_nobao"](k)
    cs *= c["aperp"] ** 2.0 * c["apar"]
    return cs * c["Tb"] ** 2.0 / (c["r"] ** 2.0 * c["rnu"])
...
def Cfg(q, y, cosmo, expt):
    ...
    nu = expt["nu_line"] / (1.0 + cosmo["z"])  # Centre freq. of redshift bin

    # Replace zeros in q to prevent div/0
    _q = q.copy()
    _q[np.where(q == 0.0)] = 1e-100  # Set to a very small (but non-zero) value
    ...
    for i in range(n_fg):
        Cx = (
            fg["A"][i]
            * (fg["l_p"] / (2.0 * np.pi * _q)) ** fg["nx"][i]
            * (nu / fg["nu_p"]) ** fg["mx"][i]
        )
        Cx[np.where(q == 0.0)] = INF_NOISE  # Mask values where q is zero
        Cfg += Cx

    # Scale by FG subtraction residual amplitude and return
    return expt["epsilon_fg"] ** 2.0 * Cfg
...
def n_IM(kgrid, ugrid, cosmo, expt):
    ...
    K, U = np.meshgrid(kgrid, ugrid)
    y = cosmo["rnu"] * K * U
    q = cosmo["r"] * K * np.sqrt(1.0 - U**2.0)
    dn_dk = K**2.0 / (Cnoise(q, y, cosmo, expt) + Cfg(q, y, cosmo, expt))
    n_z = integrate_grid(dn_dk, kgrid, ugrid) / (2.0 * np.pi) ** 2.0
    n_z *= cosmo["Tb"] ** 2.0 / (cosmo["r"] ** 2.0 * cosmo["rnu"])

    # Calculate Vsurvey
    _z = np.linspace(zmin, zmax, 1000)
    Vsurvey = C * scipy.integrate.simps(rr(_z) ** 2.0 / HH(_z), _z)
    Vsurvey *= (4.0 * np.pi) * expt["Sarea"] / (4.0 * np.pi)
...
def fisher_integrands(
    ...
    K, U = np.meshgrid(kgrid, ugrid)
    y = rnu * K * U
    q = r * K * np.sqrt(1.0 - U**2.0)

    # Calculate k, mu
    k = np.sqrt((aperp * q / r) ** 2.0 + (apar * y / rnu) ** 2.0)
    u2 = y**2.0 / (y**2.0 + (q * rnu / r * aperp / apar) ** 2.0)
    ...
    if transfer_fn is None:
        ...
        alpha = (
            (100.0 * c["h"]) ** 2.0 * c["omega_M_0"] * delta_c * Tfn * Dinv / (C**2.0)
        )
        alpha[np.where(np.isinf(alpha))] = 0.0  # Remove infs outside interp. range
        b = c["btot"] = c["bHI"] * s_k + 3.0 * (c["bHI"] - 1.0) * alpha * fNL
    ...
    if RSD_FUNCTION == "kaiser":
        drsd_df = 2.0 * u2 / (b + f * u2)
        drsd_dfsig8 = 2.0 * u2 * Dinv / (b * c["sigma_8"] + f * c["sigma_8"] * u2)
        drsd_dsig2 = -(k**2.0) * u2
        drsd_du2 = 2.0 * f / (b + f * u2) - (k * c["sigma_nl"]) ** 2.0
        drsd_dk = -2.0 * k * u2 * c["sigma_nl"] ** 2.0
    else:
        drsd_df = u2 * (
            2.0 * u2 / (b + f * u2) - (1.0 + f) * (k * c["sigma_nl"] * D) ** 2.0
        )
        drsd_dsig2 = -0.5 * (k * D) ** 2.0 * (1.0 - u2 + u2 * (1.0 + f) ** 2.0)
        drsd_du2 = 2.0 * f / (b + f * u2) - 0.5 * (k * c["sigma_nl"] * D) ** 2.0 * (
            (1.0 + f) ** 2.0 - 1.0
        )
        drsd_dk = -k * (D * c["sigma_nl"]) ** 2.0 * (1.0 - u2 + u2 * (1.0 + f) ** 2.0)
    ...
    if transfer_fn is not None:
        ...
        # Derivatives of b_NG
        dbng_fNL = 3.0 * (b - 1.0) * alpha
        dbng_bHI = s_k + 3.0 * alpha * fNL  # Used in deriv_bHI
        dbng_k = -3.0 * (b - 1.0) * alpha * fNL * dTfn_dk  # (2./k + dTk_k/Tk)
        dbng_f = -6.0 * (b - 1.0) * alpha * fNL * np.log(a)

        # Extra terms used in EOS expansion
        dbng_omegak = -3.0 * (b - 1.0) * alpha * fNL / c["omega_M_0"]
        ...
        if massive_nu_fn is not None:
            print(
                "\tWARNING: (M_nu, f_NL) crossterm not currently supported."
                "Setting to 0."
            )
            """
        ...
        # Derivatives of C_S
        dbias_k = (
            2.0 * c["bHI"] * (2.0 * c["b_1"] * k / c["k0_bias"] ** 2.0) / (b + f * u2)
            + dbng_k * fac
        )
        ...
        dbng_bHI = s_k
        dbias_k = (
            2.0 * c["bHI"] * (2.0 * (k / c["k0_bias"] ** 2.0) * c["b_1"]) / (b + f * u2)
        )
        dbng_df_term = 0.0
        dbng_dmnu_term = 0.0
        deriv_omegak_ng = 0.0
        deriv_omegaDE_ng = 0.0

    # Get analytic log-derivatives for parameters
    # FIXME: (f sigma8) and (b sigma8) terms don't handle NG properly
    deriv_A = c["fbao"](k) / (1.0 + c["A"] * c["fbao"](k)) * cs / ctot
    deriv_bHI = 2.0 * dbng_bHI / (b + f * u2) * cs / ctot
    deriv_f = (
        (
            use["f_rsd"]
            * drsd_df  # + use['f_growthfactor'] * 2.*np.log(a) \ # This is wrong!
            + dbng_df_term
        )
        * cs
        / ctot
    ...
    deriv_bsig8 = (
        2.0 * dbng_bHI * Dinv / (b * c["sigma_8"] + f * c["sigma_8"] * u2) * cs / ctot
    )
    ...
    deriv_b1 = 2.0 * c["bHI"] * (k / c["k0_bias"]) ** 2.0 / (b + f * u2) * cs / ctot

    # Get analytic log-derivatives for additional parameters
    deriv_sigma8 = (2.0 / c["sigma_8"]) * cs / ctot
    deriv_ns = np.log(k / c["k_piv"]) * cs / ctot
    deriv_Tb = (2.0 / c["Tb"]) * cs / ctot if not galaxy_survey else 0.0

    # Analytic log-derivatives for modified gravity parameters
    a = 1.0 / (1.0 + c["z"])
    Oma = c["omega_M_0"] * (1.0 + c["z"]) ** 3.0 / Ez(cosmo, c["z"]) ** 2.0
    xi = 3.0 * (100.0 * c["h"] / C) ** 2.0 * c["omega_M_0"] * (1.0 + c["z"]) / k**2.0
    eta = 0.0 if "eta0" not in list(c.keys()) else c["eta0"] + c["eta1"] * (1.0 - a)
    ...
    if "oldmg" in switches:
        ...
        deriv_gamma1 = deriv_f * f * np.log(Oma) * (1.0 - a)
        deriv_eta0 = deriv_f * f / (1.0 + eta)
        deriv_eta1 = deriv_f * f * (1.0 - a) / (1.0 + eta)

        # Horndeski-inspired growth modification
        deriv_Axi, deriv_logkmg = mg_growth.growth_derivs(
            c["z"], K, c, mg_params=["A_xi", "logkmg"], dx=[1e-3, 1e-1]
        )
        deriv_Axi *= deriv_f
    ...
    if "f0_kbins" in list(c.keys()):
        for i in range(len(c["f0_kbins"]) - 1):
            ...
            derivs_f0k[-1][np.where(np.logical_or(k < ka, k >= kb))] = 0.0
    ...
    if "fs8_kbins" in list(c.keys()):
        for i in range(len(c["fs8_kbins"]) - 1):
            ...
            kb = c["fs8_kbins"][i + 1]
            msk = np.ones(k.shape)
            msk[np.where(np.logical_or(k < ka, k >= kb))] = 0.0
    ...
    daperp_u2 = -2.0 * (rnu / r * q / y * aperp / apar * u2) ** 2.0 / aperp
    dapar_u2 = 2.0 * (rnu / r * q / y * aperp / apar * u2) ** 2.0 / apar
    daperp_k = (aperp * q / r) ** 2.0 / (k * aperp)
    dapar_k = (apar * y / rnu) ** 2.0 / (k * apar)
    ...
    if use["alpha_all"]:
        deriv_aperp = (
            (
                (2.0 / aperp)
                + drsd_du2 * daperp_u2
                + (dlogpk_dk + drsd_dk + dbias_k) * daperp_k
            )
            * cs
            / ctot
        )
        deriv_apar = (
            (
                (1.0 / apar)
                + drsd_du2 * dapar_u2
                + (dlogpk_dk + drsd_dk + dbias_k) * dapar_k
            )
            * cs
            / ctot
        )
        ...
        t_term[3] = c["A"] * dfbao_dk / (1.0 + c["A"] * c["fbao"](k)) * daperp_k
        ...
        r_term[3] = c["A"] * dfbao_dk / (1.0 + c["A"] * c["fbao"](k)) * dapar_k
...
def eos_fisher_matrix_derivs(cosmo, cosmo_fns, fsigma8=False):
    ...
    omegaDE = lambda a: ol * np.exp(3.0 * wa * (a - 1.0)) / a ** (3.0 * (1.0 + w0 + wa))
    E = lambda a: np.sqrt(om * a ** (-3.0) + ok * a ** (-2.0) + omegaDE(a))

    # Derivatives of E(z) w.r.t. parameters
    # dE_omegaM = lambda a: 0.5 * a**(-3.) / E(a)
    if np.abs(ok) < 1e-7:  # Effectively zero
        dE_omegak = lambda a: 0.5 * a ** (-2.0) / E(a)
    else:
        dE_omegak = lambda a: 0.5 * a ** (-2.0) / E(a) * (1.0 - 1.0 / a)
    dE_omegaM = lambda a: 0.5 * a ** (-3.0) / E(a)
    dE_omegaDE = lambda a: 0.5 / E(a) * (1.0 - 1.0 / a**3.0)
    dE_w0 = lambda a: -1.5 * omegaDE(a) * np.log(a) / E(a)
    dE_wa = lambda a: -1.5 * omegaDE(a) * (np.log(a) + 1.0 - a) / E(a)
    ...
    r_c = scipy.integrate.cumtrapz(1.0 / (aa**2.0 * EE), aa)
    r_c = np.concatenate(([0.0], r_c))
    if ok > 0.0:
    ...
    derivs_aperp = [
        (C / H0) / r[1:] * scipy.integrate.cumtrapz(f(aa) / (aa * EE) ** 2.0, aa)
        for f in fns
    ]
    ...
    inivals = [0.5, 0.0, 0.0, 0.0]
    derivs_aperp = [
        np.concatenate(([inivals[i]], derivs_aperp[i]))
    ...
    interp_f = [
        scipy.interpolate.interp1d(aa[::-1], d[::-1], kind="linear", bounds_error=False)
        for d in derivs_f
    ]
    interp_apar = [
        scipy.interpolate.interp1d(aa[::-1], d[::-1], kind="linear", bounds_error=False)
        for d in derivs_apar
    ]
    interp_aperp = [
        scipy.interpolate.interp1d(aa[::-1], d[::-1], kind="linear", bounds_error=False)
        for d in derivs_aperp
    ]
...
def add_fisher_matrices(F1, F2, lbls1, lbls2, info=False, expand=False):
    ...
    if expand:
        ...
        if info:
            print(
                "add_fisher_matrices: Expanded output matrix to include non-overlapping params:",
                not_found,
            )
        ...
        if lbls2[ii] in lbls1:
            for jj in range(len(lbls2)):
                if lbls2[jj] in lbls1:
                    ...
                    Fnew[_i, _j] += F2[ii, jj]
                    if info:
                        print(lbls1[_i], lbls2[ii], "//", lbls1[_j], lbls2[jj])
...
def transform_to_lss_distances(
    z, F, paramnames, cosmo_fns=None, DA=None, H=None, rescale_da=1.0, rescale_h=1.0
):
    """
    ...
    if cosmo_fns is not None:
        ...
        DA = rr(z) / (1.0 + z)
        H = HH(z)
    DV = ((1.0 + z) ** 2.0 * DA**2.0 * C * z / H) ** (1.0 / 3.0)
    FF = (1.0 + z) * DA * H / C
    ...
    S[ida, iff] = DA / (3.0 * FF)  # d D_A / d F
    S[ih, idv] = -H / DV  # d H / d D_V
    S[ih, iff] = 2.0 * H / (3.0 * FF)  # d H / d F
...
def fisher_with_excluded_params(F, excl, names):
    ...
    for n in excl_names:
        del new_names[new_names.index(n)]
    return Fnew, new_names
...
def indices_for_param_names(paramnames, params, warn=True):
    ...
    if type(params) is str:
        params = [
            params,
        ]  # Convert single param. string to list
    idxs = []
    for p in params:
        if p[-1] == "*":
            ...
            if p in paramnames:
                ...
                if warn:
                    print(
                        "\tindices_for_param_names(): %s not found in param list." % p
                    )
...
def fisher(
    ...
    if (expt["mode"][0] == "i") and "n(x)" in list(
        expt.keys()
    ):  # FIXME: Fails for combined
    ...
    cosmo["r"] = rr(z)
    cosmo["rnu"] = C * (1.0 + z) ** 2.0 / HH(z)  # Perp/par. dist. scales
    cosmo["switches"] = switches

    # Physical volume (in rad^2 Mpc^3) (note factor of nu_line in here)
    Vphys = (
        expt["Sarea"]
        * (expt["dnutot"] / expt["nu_line"])
        * cosmo["r"] ** 2.0
        * cosmo["rnu"]
    )
    Vfac = np.pi * Vphys / (2.0 * np.pi) ** 3.0

    # Set-up integration sample points in (k, u)-space
    ugrid = np.linspace(-1.0, 1.0, NSAMP_U)  # N.B. Order of integ. limits is correct
    ...
    # Output k values
    c = cosmo
    D0 = 0.5 * 1.02 * 300.0 / np.sqrt(2.0 * np.log(2.0))  # Dish FWHM prefactor [metres]
    kfg = 2.0 * np.pi * expt["nu_line"] / (expt["survey_dnutot"] * c["rnu"])
    sigma_kpar = (2.0 * np.pi) * expt["nu_line"] / (expt["dnu"] * c["rnu"])
    sigma_kperp = (
        np.sqrt(2.0) * expt["Ddish"] * expt["nu_line"] / (c["r"] * D0 * (1.0 + c["z"]))
    )
    print("-" * 50)
    print("kmin\t  %s" % kmin)
    print("kmax\t  %s" % kmax)
    ...
    print(
        "lmin \t %4.1f" % (2.0 * np.pi / np.sqrt(expt["Sarea"]))
    )  # FIXME: Should be FOV for interferom.
    print("lmax \t %4.1f" % (sigma_kperp * c["r"]))
    print("signl\t %4.4f" % (1.0 / cosmo["sigma_nl"]))
    ...
    if return_pk:
        ...
        pnew = len(cumul) - 1
        FF, paramnames = fisher_with_excluded_params(
            F, excl=[F.shape[0] - 1], names=paramnames
        )
        ...
        binning_info = {"F_base": FF, "Vfac": Vfac, "cumul": cumul, "kgrid": kgrid}
    ...
    # Return results
    if return_pk:
        return F_pk, kc, binning_info, paramnames
    return F, paramnames

...
# 用户自定义 k 下限旋钮（各向同性，作用于 k = sqrt(kpar^2 + kperp^2)）
#   KMIN_EFF = 0.0 : 关闭（默认，沿用物理截止：文件 n(u)/Dmin/FOV/前景楔）
#   KMIN_EFF > 0.0 : 启用自定义模式；4 处物理低 k 截止全部关闭，
#                    仅保留 k < KMIN_EFF 的掩码（所有方向一视同仁）
#   notebook 用法：bf.KMIN_EFF = 1e-3
KMIN_EFF = 0.0

# 用户自定义 k 上限旋钮（各向同性，作用于 k = sqrt(kpar^2 + kperp^2)）
#   KMAX_EFF = 0.0 : 关闭（默认，沿用物理截止：非线性尺度 0.14(1+z)^(2/(2+ns))、
#                     uniform 干涉的 Dmax 基线上限、文件 n(u) 覆盖范围）
#   KMAX_EFF > 0.0 : 启用自定义模式；高 k 端物理截止全部关闭，
#                     仅保留 k > KMAX_EFF 的掩码（所有方向一视同仁）
#   notebook 用法：bf.KMAX_EFF = 0.35
KMAX_EFF = 0.0
...
def load_interferom_file(fname):
    ...
    if KMIN_EFF > 0.0:
        # 自定义模式：文件越界用边缘值填充（关闭低 u 物理截止）
        interp_nx = scipy.interpolate.interp1d(
            x, _nx, kind="linear", bounds_error=False, fill_value=(_nx[0], _nx[-1])
        )
    else:
        # 默认模式：文件越界视为无基线覆盖（1/INF_NOISE）
        interp_nx = scipy.interpolate.interp1d(
...
def interferometer_response(q, y, cosmo, expt):
    ...
    if "n(x)" in list(expt.keys()):
        ...
        if (KMIN_EFF > 0.0) or (KMAX_EFF > 0.0):
            # 自定义模式：关闭 Dmin 低 u 截止；Dmax 高 u 上限仅在
            # 未启用 KMAX_EFF 时保留（否则由 k > KMAX_EFF 各向同性掩码统一接管）
            if KMAX_EFF <= 0.0:
    ...
    # FOV cut-off (disabled for cylinders; also disabled in custom mode)
    if ("cyl" not in expt["mode"]) and (KMIN_EFF <= 0.0):
        l = 3e8 / (nu * 1e6)  # Wavelength (m)
...
def Cnoise(q, y, cosmo, expt, cv=False):
    ...
    if KMIN_EFF > 0.0:
        # 自定义模式：关闭前景楔物理截止，改用各向同性 k 下限掩码
        # k = sqrt(kpar^2 + kperp^2) < KMIN_EFF 的所有模式置为无穷噪声
        noise[np.where(np.sqrt(kpar**2.0 + kperp**2.0) < KMIN_EFF)] = INF_NOISE
    else:
        # 默认模式：平行方向频域前景扣除截止（前景楔）
        kfg = 2.0 * np.pi * expt["nu_line"] / (expt["survey_dnutot"] * c["rnu"])
        kfg *= expt["kfg_fac"] if "kfg_fac" in list(expt.keys()) else 1.0
        noise[np.where(np.abs(kpar) < kfg)] = INF_NOISE

    # Cut off NL scales by adding INF_NOISE
    if KMAX_EFF > 0.0:
        # 自定义模式：关闭非线性尺度物理截止，改用各向同性 k 上限掩码
        # k = sqrt(kpar^2 + kperp^2) > KMAX_EFF 的所有模式置为无穷噪声
        noise[np.where(np.sqrt(kpar**2.0 + kperp**2.0) > KMAX_EFF)] = INF_NOISE
    else:
        # 默认模式：非线性尺度截止（Smith et al. 2003, Eq. 20）
        if "k_nl0" not in list(expt.keys()):
            print("\tNonlinear scale not specified; setting k_NL,0 = 0.14 Mpc^-1")
            knl0 = 0.14
        else:
            knl0 = expt["k_nl0"]
        kmax = knl0 * (1.0 + c["z"]) ** (2.0 / (2.0 + c["ns"]))

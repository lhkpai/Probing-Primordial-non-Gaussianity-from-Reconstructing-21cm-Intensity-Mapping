...
def camb_params(
    ...
    pivot_scalar=0.05,
    pivot_tensor=0.05,
    scalar_amp__1___=2.1e-9,
    scalar_spectral_index__1___=0.96,
    scalar_nrun__1___=0,
    tensor_spectral_index__1___=0,
    initial_ratio__1___=1,
    reionization="T",
    ...
    transfer_filename__1___="transfer_out.dat",
    ...
    bispectrum_ndelta=3,
    bispectrum_delta__1___=0,
    bispectrum_delta__2___=2,
    bispectrum_delta__3___=4,
...
def run_camb(params_fname, camb_exec_dir):
    ...
    """
    # Resolve camb binary location.
    # `camb_exec_dir` can be either the directory containing the executable,
    # or the full path to the executable itself.
    camb_path = Path(camb_exec_dir).expanduser()
    if camb_path.is_file():
        camb_bin = camb_path
        camb_dir = camb_path.parent
    elif camb_path.is_dir():
        camb_dir = camb_path
        camb_bin = camb_dir / "camb"
    else:
        raise FileNotFoundError(
            f"CAMB execution directory not found: {camb_exec_dir}. "
            "Set CAMB_EXEC_DIR (or BAO21CM_CAMB_EXEC) to the directory that contains the 'camb' executable "
            "(e.g. '/home/liuhk/CAMB/fortran' after building CAMB)."
        )

    if not camb_bin.exists():
        raise FileNotFoundError(
            f"CAMB executable not found at: {camb_bin}. "
            "You likely need to compile CAMB first (e.g. 'cd /home/liuhk/CAMB/fortran && make'). "
            "Then set CAMB_EXEC_DIR to that directory."
        )

    # Call CAMB without changing the process working directory.
    cwd = os.getcwd()
    params_path = cwd + "/paramfiles/" + params_fname
    print("Running CAMB on", params_path)
    output = subprocess.check_output([str(camb_bin), params_path], cwd=str(camb_dir))
...
def comoving_dist(a, cosmo):
    ...
    """
    aa = np.logspace(np.log10(a), 0.0, 1000)
    zz = 1.0 / aa - 1.0
    ...
    w0 = cosmo["w0"]
    wa = cosmo["wa"]
    ombh2 = cosmo["omega_b_0"] * cosmo["h"] ** 2.0
    om = cosmo["omega_M_0"]
    ol = cosmo["omega_lambda_0"]
    ogam = 2.47e-5 / cosmo["h"] ** 2.0  # Rad. density fraction, from Dodelson Eq. 2.70
    Neff = 3.046
    onu = (7.0 / 8.0) * (4.0 / 11.0) ** (4.0 / 3.0) * Neff * ogam
    ok = 1.0 - om - ol

    # Omega_DE(z) and 1/E(z)
    omegaDE = ol  # * np.exp(3.*wa*(aa - 1.)) / aa**(3.*(1. + w0 + wa))
    invE = 1.0 / np.sqrt(
        om * aa + ok * aa**2.0 + ogam + onu + omegaDE * aa**4.0
    )  # 1/(a^2 H)
...
def rsound(a, cosmo):
    ...
    aa = np.logspace(-8.0, np.log10(a), 1000)
    ...
    # Omega_DE(z) and E(z)
    omegaDE = ol * np.exp(3.0 * wa * (aa - 1.0)) / aa ** (3.0 * (1.0 + w0 + wa))

    # Integrate sound speed
    R = 3.0364e4 * ombh2 * aa  # Baryon-photon ratio
    cs = np.sqrt(3.0 + 3.0 * R)  # Sound speed
    rs_integ = 1.0 / np.sqrt(
        om * aa + ok * aa**2.0 + ogam + onu + omegaDE * aa**4.0
    )  # 1/(a^2 H)
    rs_integ /= cs
...
def cmb_to_theta(cosmo, h):
    ...
    zstar = (
        1048.0
        * (1.0 + 0.00124 * ombh2**-0.738)
        * (
            1.0
            + (0.0783 * ombh2**-0.238 / (1.0 + 39.5 * ombh2**0.763))
            * (omch2 + ombh2) ** (0.560 / (1.0 + 21.1 * ombh2**1.81))
        )
    )
    astar = 1.0 / (1.0 + zstar)
...
def find_h_for_theta100(th100, cosmo, hmin=0.5, hmax=0.85, nsamp=20):
    ...
    th = 100.0 * np.array([cmb_to_theta(c, hh) for hh in hvals])
